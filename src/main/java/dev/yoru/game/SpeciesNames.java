...
/**
 * English species names, by National Dex number.
 *
 * Names are facts rather than game assets, and the same in every English Gen 3
 * release, so they are written out here instead of being read from a ROM: the
 * study workspace has to be able to name a Pokémon on a machine with no game
 * files at all.
 */
public final class SpeciesNames {
    private SpeciesNames() { }

    /** How many species this build knows, National Dex 1 to 386. */
    public static final int COUNT = 386;

    /** Index 0 is unused, so a National Dex number indexes directly. */
    ...
    /** The name for a National Dex number from 1 to 386. */
    public static String of(int nationalDexNumber) {
        if (nationalDexNumber < 1 || nationalDexNumber > COUNT)
            throw new IllegalArgumentException("No species " + nationalDexNumber + ".");
        return NAMES[nationalDexNumber];
    }
}
