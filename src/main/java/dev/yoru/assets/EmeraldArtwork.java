...
public final class EmeraldArtwork {
    ...
    static BufferedImage[] wallpapers(byte[] rom) throws IOException {
        for(int at=0;at<=rom.length-16*12;at+=4) {
            int first=target(rom,at),map=target(rom,at+4);
            if(first<0||map<0||rom[first]!=16||rom[map]!=16||map>rom.length-4)continue;
            if((rom[map+1]&255)!=0xd0||(rom[map+2]&255)!=2||rom[map+3]!=0)continue;
            try {
                var out=new BufferedImage[16];
                for(int i=0;i<16;i++) {
                    int entry=at+i*12;
        ...
        throw new IOException("Could not find the game's box wallpapers.");
    }

    private static int target(byte[] rom,int offset) {
