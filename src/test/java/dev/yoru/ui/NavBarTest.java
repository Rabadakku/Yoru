...
public final class NavBarTest {
    ...
    private static void atSize(float scale, boolean smallest) throws Exception {
        ...
        for (var tab : all) {
            ...
            check(metrics.stringWidth(tab.getText()) <= b.width - in.left - in.right,
                tab.getText() + " is whole at " + scale + "x, " + width + " wide");
            check(b.x >= previousRight + NavTab.MIN_GAP, "Tabs never touch, at " + scale + "x");
            previousRight = b.x + b.width;
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("PASS: " + checks + " top bar checks (whole tabs at every size, roomy when wide, current marked)");
        System.exit(0);
    }
}
